...
export function ReadinessScore({ score, onClick }: ReadinessScoreProps) {
  ...
  return (
    ...
    >
      <div className="relative w-16 h-16 flex-shrink-0">
        ...
        <span
          className={`absolute inset-0 flex items-center justify-center text-sm font-bold ${getScoreColor(score.score)}`}
        >
          {score.score}%
        </span>
      </div>
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-zinc-500 mt-0.5">
          {score.reviewedCards}/{score.totalCards} Karten gelernt
        </p>
      </div>
    </button>
  )
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="relative w-32 h-32">
        ...
        <span
          className={`absolute inset-0 flex items-center justify-center text-2xl font-bold ${getScoreColor(score)}`}
        >
          {score}%
        </span>
      </div>
      <p className="text-zinc-500 text-sm mt-2">Gesamtbereitschaft</p>
    </div>
  )
}
